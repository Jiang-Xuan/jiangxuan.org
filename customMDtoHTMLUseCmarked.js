...
let cmarked = require('cmarked') // 引入个人markdown编译器
let fs = require('fs') // 引入fs模块

// 定义转换markdown到HTML的插件类
function customMDtoHTMLUseCmarked(option) {
	// 处理从实例传进来的选项
...
customMDtoHTMLUseCmarked.prototype.apply = function (compiler) {
	let self = this
	// emit事件为webpack即将生成文件之前触发的事件
	compiler.plugin('emit', (compilation, callback) => {
		// 编译markdown内容
		let content = cmarked(self.option.nonTransformedMD)
		// ---------------------- 处理content ---------------------------
...
// 导出类
module.exports = customMDtoHTMLUseCmarked
